#!/usr/bin/env python3
"""
RTC Season 2020.3 Import Script - Horizontal Format
...
Verwendung:
    python3 rtc_import_2020_3.py "data/RTC_2020.3_Kopie - Races.csv" "data/RTC_2020.3_Kopie - Streams.csv"

Environment-Variablen (erforderlich):
    DB_HOST, DB_PORT, DB_NAME, DB_USER, DB_PASSWORD
    SEASON_ID=4 (für Season 2020.3)

Unterschiede zu 2021.1:
- Streams.csv: Nummer in B (index 1), Datum in C (index 2), Track in D (index 3)
- Grid-Klassen: 1, 2a, 2b, 2c, 3 (direkt übernehmen)
- Andere Track-Namen (deutsche/lange Namen)
"""
...
# Track-Mapping
TRACK_NAME_MAP = {
    'Autodromo Nazionale Monza': ('Monza', 'Full Course'),
    'Autodromo de Interlagos': ('Autódromo De Interlagos', 'Full Course'),
    'Brands Hatch Indy': ('Brands Hatch', 'Indy Circuit'),
    'Circuit de Barcelona-Catalunya': ('Barcelona', 'GP'),
    'Circuit de Spa-Francorchamps': ('Spa-Francorchamps', 'Full Course'),
    'Circuit de la Sarthe (Le Mans)': ('LeMans', 'Full Course'),
    'Dragon Trail - Gärten II': ('Dragon Trail', 'Gardens Reverse'),
    'Fuji International Speedway': ('Fuji International Speedway', 'Full Course'),
    'Mount Panorama Motor Racing Circuit': ('Mount Panorama', 'Full Course'),
    'Nürburgring 24h': ('Nürburgring', '24h Layout'),
    'Nürburgring GP': ('Nürburgring', 'GP'),
    'Red Bull Ring': ('Red Bull Ring', 'Full Course'),
    'Sardegna Strecke B': ('Sardegna - Road Track', 'B'),
    'Suzuka Circuit': ('Suzuka Circuit', 'Full Course'),
    'Tokyo Expressway - Östlicher Innenring': ('Tokyo Expressway', 'East Clockwise'),
    'WeatherTech Raceway Laguna Seca': ('Laguna Seca', 'Full Course'),
}
...
class Season2020_3Importer:
    """Import für Season 2020.3 mit horizontalem Format"""
    ...
    def parse_streams_csv(self) -> Dict[int, Dict]:
        ...
        Season 2020.3: Nummer in B (index 1), Datum in C (index 2), Track in D (index 3)
        """
    ...
    def parse_race_results(self, data_rows: List[List[str]], start_col: int) -> List[Dict]:
        ...
        for row in data_rows[5:MAX_DATA_ROW]:
            ...
            # +8: CL (ignorieren), +9: Grid (1, 2a, 2b, 2c, 3)
    ...
    def run(self):
        """Hauptprozess"""
        print("="*60)
        print("RTC Season 2020.3 Import - Horizontal Format")
        print("="*60)
...
def main():
    if len(sys.argv) != 3:
        print('Usage: python3 rtc_import_2020_3.py <races.csv> <streams.csv>')
        sys.exit(1)
    ...
    try:
        importer = Season2020_3Importer(races_csv, streams_csv)
        importer.run()
