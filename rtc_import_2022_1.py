#!/usr/bin/env python3
"""
RTC Season 2022.1 Import Script - Horizontal Format
...
Verwendung:
    python3 rtc_import_2022_1.py data/RTC_2022_1_Kopie_-_Races.csv data/RTC_2022_1_Kopie_-_Streams.csv

Environment-Variablen (erforderlich):
    DB_HOST, DB_PORT, DB_NAME, DB_USER, DB_PASSWORD
    SEASON_ID=7 (für Season 2022.1)
"""
...
class Season2022_1Importer:
    """Import für Season 2022.1 mit horizontalem Format"""
    ...
    def parse_streams_csv(self) -> Dict[int, Dict]:
        ...
        with open(self.streams_csv, 'r', encoding='utf-8') as f:
            ...
            # Zeile 3+ haben Daten (Zeile 0-2 = Header)
            for row in rows[3:]:
                if len(row) < 6:
                    continue
    ...
    def get_race_column_ranges(self) -> List[Tuple[int, int, int]]:
        """Bestimme Spalten-Bereiche für jedes Rennen
        Returns: [(race_num, start_col, end_col), ...]
        """
        # Jedes Rennen braucht ~18 Spalten
        # Race 1: Spalten 1-18 (0-based)
        # Race 2: Spalten 19-36
        # usw.
    ...
    def parse_time(self, time_str: str) -> Optional[str]:
        """Parse Zeitformat 0:51:07.195 oder DNF"""
        if not time_str or time_str == 'DNF':
            return None
        
        # Format: 0:51:07.195 oder 0:51:07,195
    ...
    def parse_fastest_lap(self, sr_row: List[str], start_col: int) -> Tuple[Optional[str], Optional[str]]:
        ...
        Returns: (driver_name, lap_time)
        """
        try:
            # SR-Zeile hat: SR, NAT, DRIVER, CAR, , LAPTIME, ...
            driver_col = start_col + 2
    ...
    def parse_race_results(self, data_rows: List[List[str]], start_col: int, end_col: int) -> List[Dict]:
        """Parse Ergebnisse für ein Rennen aus den Spalten"""
        results = []
    ...
    def process_all_races(self):
        """Verarbeite alle 16 Rennen"""
        
        # Parse Streams für Datum+Track
        ...
        # Zeile 4 = SR (schnellste Runde)
        sr_row = rows[3]
        
        # Spalten-Bereiche
        race_ranges = self.get_race_column_ranges()
        
        for race_num, start_col, end_col in race_ranges:
            ...
            # Ergebnisse parsen
            results = self.parse_race_results(rows, start_col, end_col)
    ...
    def insert_new_drivers_and_teams(self, results: List[Dict]):
        ...
        for r in results:
            ...
            # Check driver
            if r['driver'] not in self.drivers:
        ...
        for driver_name, nat in new_drivers:
            self.cursor.execute("INSERT IGNORE INTO drivers (psn_name, nat) VALUES (%s, %s)", 
    ...
    def calculate_time_percent(self, results: List[Dict]):
        ...
        # Finde schnellste Zeit (ohne DNFs)
        time_seconds = []
        for r in results:
            if r['race_time']:
    ...
    def insert_results(self, results: List[Dict], grid_map: Dict[str, int]):
        ...
        for r in results:
            ...
            # Parse Penalty
            penalty_seconds = 0
            if r['penalty'] and r['penalty'].isdigit():
                penalty_seconds = int(r['penalty'])
            
            # Status
            status = 'DNF' if r['race_time'] is None else 'FIN'
            
            # time_percent (falls berechnet)
            ...
            """, (self.race_id, driver_id, team_id, vehicle_id, grid_id,
                  r['pos'], r['race_time'], penalty_seconds, status, time_percent))
    ...
    def run(self):
        """Hauptprozess"""
        print("="*60)
        print("RTC Season 2022.1 Import - Horizontal Format")
        print("="*60)
...
def main():
    if len(sys.argv) != 3:
        print("Usage: python3 rtc_import_2022_1.py <races.csv> <streams.csv>")
        sys.exit(1)
    ...
    try:
        importer = Season2022_1Importer(races_csv, streams_csv)
        importer.run()
