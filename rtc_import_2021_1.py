#!/usr/bin/env python3
"""
RTC Season 2021.1 Import Script - Horizontal Format
...
Verwendung:
    python3 rtc_import_2021_1.py "data/RTC_2021.1_Kopie - Races.csv" "data/RTC_2021.1_Kopie - Streams.csv"

Environment-Variablen (erforderlich):
    DB_HOST, DB_PORT, DB_NAME, DB_USER, DB_PASSWORD
    SEASON_ID=5 (für Season 2021.1)

Unterschiede zu 2021.2:
- Grid-Klasse in offset +9 (statt +8)
- Grid-Werte: 1, 2a, 2b, 2c, 3 (direkt übernehmen)
- CL (offset +8) ist immer 'Gast', wird ignoriert
- Datum in Streams.csv in Spalte D (index 3), Track in Spalte E (index 4)
"""
...
# Spalten-Abstand zwischen Rennen (Season 2021.1)
RACE_COL_STEP = 17
...
class Season2021_1Importer:
    """Import für Season 2021.1 mit horizontalem Format"""
    ...
    def parse_streams_csv(self) -> Dict[int, Dict]:
        ...
        Season 2021.1: Rennnummer in Spalte C (index 2), Datum in D (index 3), Track in E (index 4)
        """
    ...
    def insert_grids(self, grid_classes: List[str]) -> Dict[str, int]:
        """Füge Grids ein - grid_class direkt übernehmen (1, 2a, 2b, 2c, 3)"""
        grid_map = {}
    ...
    def run(self):
        """Hauptprozess"""
        print("="*60)
        print("RTC Season 2021.1 Import - Horizontal Format")
        print("="*60)
...
def main():
    if len(sys.argv) != 3:
        print('Usage: python3 rtc_import_2021_1.py <races.csv> <streams.csv>')
        sys.exit(1)
    ...
    try:
        importer = Season2021_1Importer(races_csv, streams_csv)
        importer.run()
